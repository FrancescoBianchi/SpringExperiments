...
angular
    ...
    */
    ...
    ).directive('validityFieldEmit', function () { //http://shazwazza.com/post/Listening-for-validation-changes-in-AngularJS
        return {
            require: 'ngModel',
            restrict: "A",
            link: function (scope, element, attr, ctrl) {
                ...
                if (!attr.name) {
                    throw "validityFieldEmit must be set on an input element that has a 'name' attribute";
                }

                var currentForm = scope.form;
                if (!currentForm || !currentForm.$name)
                    throw "validityFieldEmit requires that a name is assigned to the ng-form containing the validated input";
